...
Then remove the repo package version (in my case, this is org-plus-congtrib) and
re-install. This will normally ensure a 'stable' environment.
...
** Org Modules
#+BEGIN_SRC emacs-lisp
  (setq org-modules '(org-bibtex org-docview org-eww org-info org-irc
                                 org-bookmark org-drill org-git-link
                                 org-secretary))
...
Some of the diagram support features of org require external packages. This
includes
  - ditaa :: An ascii based diagram generator which uses the =ditaa.jar=
             library. This library is located in =~/.emacs.d/jars= and has been
             checked into the =emacs-init-org= repository.
  - dot :: A text based state or data flow diagram feature based on the
           =graphviz= software. This software needs to be installed separately.
  - plantUML :: A UML diagram feature based on the =plantUML.jar= library. The
                jar file has been checked into the =emacs-init-org= repository
                in the =~/.emacs.d/jars= directory.
...
I like to tweak the look of exported latex documents from the default by using
the ~hitec~ class as most of my documents are technical in nature. I also define
a couple of new document classes

  - article :: Uses the ~hitec~ document class
  - old-article :: This is the default original org-mode definition
  - une-article :: One specific to work
  - une-logo :: Includes the UNE logo and colours 
...
I don't like the default /box/ link style used by the hyperref package. Looks a
little ugly to me. Prefer instead to use coloured links to indicate hyper refs.
...
By default, org uses the verbatim environment for code blocks. I prefer to use
the listings package and add a little colour. At some point, need to investigate
the ~minted~ package as an alternative.
...
I think the ~lualatex~ seems to work better when working in a UTF-8
environment. Therefore, set the PDF process commands to use it. 
...
Need to add a few additional packages to make all of this work. In particular,
the ~xcolor~ and ~parskip~ packages. 
...
#+END_SRC
** Finally
Add require statements and do some things which need to be done after org is
loaded. 
...
#+END_SRC
* Org Key Bindings
Put the org key bindings after loading of use-package, which also
installs bind-key
