...
** Completions 
   Trying a new completions approach. Now combine hippie expand, yasnippets,
   company and flyspell on a single key with a custom function to control what
   it does. Will probably require tweaking.
*** Yasnippet
    #+BEGIN_SRC emacs-lisp
