...
#+BEGIN_SRC emacs-lisp  (defvar url-http-extra-headers nil)
...
Setup seepchd-el, the speech dispatcher interface for emacs. This is an
alternative, more light-weight speech interface to emacspeak. While it has a
much lighter impact on Emacs, it also lacks some of the advanced features of
emacspeak and requires more customization to become as productive. The advantage
is that it is easy to customize and enables creation of a bespoke environment
tailored to individual needs.

#+begin_src emacs-lisp
  (add-to-list 'load-path (expand-file-name "~/git/speechd-el"))

  (require 'info)
  (add-to-list 'Info-additional-directory-list
           (expand-file-name "~/git/speechd-el"))

  (require 'speechd-speak)
  (require 'speechd-mu4e)

  (setq speechd-default-sound-priority 'notification
        speechd-default-char-priority 'text
        speechd-default-key-priority 'text
        speechd-out-active-drivers '(ssip)
        speechd-speak-auto-speak-buffers '("*Help*")
        speechd-speak-buffer-insertions nil
        speechd-speak-buffer-name t
        speechd-speak-by-properties-on-movement nil
        speechd-speak-on-minibuffer-exit nil
        speechd-speak-read-command-keys nil
        speechd-speak-signal-events '(empty minibuffer)
        speechd-speak-state-changes nil)

  (add-hook 'speechd-speak-hook (lambda ()
                                  (speechd-set-rate 70)))
  (speechd-speak)
...
#+BEGIN_SRC emacs-lisp
  (if *is-a-mac*
    ...
    (if (string-equal (system-name) "arch-vbox")
        (progn
          (message "Set font for Arch Vbox")
          (set-face-attribute 'default nil
                              :foundry "CTDB"
                              :family "Fira Code"
                              :height 188))
      (progn
        (message "Set font for Linux desktop")
        (set-face-attribute 'default nil
                            :foundry "CTDB"
                            :family "Fira Code"
                            :height 242))))
...
Setup some font scaling support. Drop this package for now.
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package night-owl-theme
    ...
    (setq-default night-owl-background "#011627"
                  night-owl-background-highlight "red4"
